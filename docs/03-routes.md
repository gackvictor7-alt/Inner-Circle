...
| URL | Zweck | Zugriff | Status | Daten | Berechtigungen / Hinweise |
| --- | ----- | ------- | ------ | ----- | ------------------------- |
| `/onboarding/interests` | Interessen/Ziele + Start der 48-h-Discovery-Demo | **free** (verifiziert) | WORKING | `Profile` (onboardingCompletedAt), `UserInterest`, `UserGoal`, `Trial` | nur ohne abgeschlossenes Onboarding (`onboardingComplete` ⇒ `/app`); mind. 3 Interessen; Demo genau einmal (Sprint 11: Demo-Semantik, `04-auth-membership.md` §3) |
...
| `/app/network` | Mitgliederverzeichnis mit Suche/**Rolle**/**Standort**/Interessen (Sprint 7) + **View-Segmente `?view=connections\|requests`** (Sprint 8: Alle / Verbindungen / Anfragen – Connections, offene Anfragen und andere Profile getrennt); echte Mitglieder zuerst, darunter die klar als DEMO-PROFIL markierten Beispielprofile, solange < 8 echte (gefilterte) Mitglieder existieren (§4a); Verbindungsstatus richtungsabhängig: gesendet → „Anfrage gesendet" + Zurückziehen, erhalten → Annehmen/Ablehnen; **Sprint 11:** Discovery-Demo (`trial`) sieht ausschließlich die 8 Demo-Profile (echte Abfrage läuft nicht); **Sprint 12:** echte Nutzer sehen **nur echte, sichtbare Teilnehmer** (keine Demo-Ergänzung mehr), ehrlicher Leerzustand, Filter Standort/Rolle/Branche/Interessen/Ziele/Ich suche/Ich biete kombinierbar + zurücksetzbar | member+ **oder aktiver Beta-Zugang** (Demo-Zweig für `trial`) | WORKING (ohne Zugang: `NetworkLocked` mit Hinweis „geschlossene Beta“) | liest `User`/`Profile`/`Follow`/`ConnectionRequest`, `Interest` + `src/lib/demo` (kein DB-Zugriff für Demo) | `listDirectoryMembers()` nutzt die gemeinsame Teilnahme-Regel (`src/lib/network/eligibility.ts`: aktiv, verifiziert, onboarded, Netzwerkzugang, `discoverable`, keine Blockierung); Standort nur bei `showLocation`; Views filtern die geladene Liste (keine extra Queries) |
| `/app/discover` | **Discover** – Business-Karten, Relevanz-Ranking, Filter `?role=&location=&radius=&industry=&interest=&goal=&lookingFor=&offering=&invest=&kind=&more=` (`goal` = Slug der Ziel-Taxonomie, Sprint 12; unbekannte Werte werden ignoriert) | member+ **oder aktiver Beta-Zugang** (Demo-Zweig für `trial`) | WORKING | `listDiscoverCandidates()` + `src/lib/discover/matching.ts` (Interessen, Ziele, Branche, Standort **inkl. Umkreis über gebündelte Städtetabelle**, Ich suche, Ich biete, Investmentinteressen); **Sprint 11:** Demo-Zweig für `trial` über `src/lib/demo/discover.ts` (gleiche Filter/gleiches Ranking, Demo-Profile, `DiscoverDeck mode="demo"`) | ohne `entitlements.networkDiscover` und ohne `demoAccess` ⇒ `NetworkLocked` (Sprint 12; vorher Redirect `/app/billing?paywall=trial`); Connect nur mit Pflichtnachricht (Demo: simulierter Dialog); **Sprint 12:** keine „% passend“-Angabe, keine Kennzahlen/Trust auf echten Karten; Demo-Deck zeigt „geschlossene Beta“ bzw. nach Beta-Ende einen ehrlichen Ende-Hinweis |
| `/app/inbox` | **Inbox** – `?tab=messages\|requests\|notifications`, Anfragen zusätzlich `?sub=requests\|sent\|connections`; `?to=<userId>` öffnet (oder legt per `ensureDirectConversation()` an) den 1:1-Chat; **Sprint 8: auf Mobile wie Messaging-App** (Header mit Person + zurück, Messages im Hauptscreen, Composer fixiert, Inbox-Chrome ausgeblendet) | free (Schreiben nur member **oder aktiver Beta-Zugang**) | WORKING | `Conversation`, `Message`, `ConnectionRequest`, `Connection`, `Notification` | ersetzt `/app/messages`, `/app/connections`, `/app/notifications` (Umleitungen); Chat nur zwischen verbundenen Konten (serverseitig); **Sprint 12:** Annehmen öffnet direkt `?tab=messages&c=<id>` (ein Chat je Paar, Anfrage-Nachricht als erste Nachricht), neue Nachrichten per Polling alle 10 s bei sichtbarem Tab, fremde `c=` ⇒ Umleitung ohne Inhalt, Verlauf ohne Zugang lesbar (Composer durch Hinweis ersetzt), Zähler: Anfragen = offene eingehende Anfragen, Badge = ungelesene Nachrichten + ungelesene Mitteilungen |
...
| `/app/investments` | Investment-Chancen (nur `approved`) | member+ (Demo-Zweig für `trial`) | WORKING | `InvestmentOpportunity`, `InvestmentInterest` | Free sieht Locked-State; Discovery-Demo sieht drei Demo-Beispiele; Beträge ohne Renditeversprechen |
...
| `/app/investments/[id]` | Detail + Absichtserklärung | member+ (Free/Demo: `LockedArea`) | WORKING | `InvestmentOpportunity`, `InvestmentInterest` | nicht freigegebene Einträge nur für Einreicher/Admin (`notFound()`) |
| `/app/events` | Events im Mitgliederbereich (**eigener Hauptbereich**) | free | WORKING | `Event`, `EventApplication` | Liste zeigt nur veröffentlichte Zustände mit Datum · Uhrzeit; Hinweis „kuratiert von INNER CIRCLE" (Mitglieder) bzw. „echte Events, Anmeldung ist Teil der Mitgliedschaft" (Free/Demo, Sprint 11); **keine Erstellung durch Mitglieder** |
| `/app/events/[slug]` | Event-Detail + Bewerbung/Abbestätigung | free | WORKING | `Event`, `EventApplication` | Bewerbung nur mit `eventsApply` (Action `membershipRequired`); Free/Demo sehen statt Formular eine Sperrkarte mit freien Plätzen (`capacity − applied/confirmed/attended`, nur bei gesetzter Kapazität) und Billing-CTA (Sprint 11) |
...
| `/app/people/[handle]` | Profil eines Mitglieds | member+ **oder aktiver Beta-Zugang** (eigenes Profil: free+) | WORKING | `Profile`, `User`, `Follow`, `Connection`, `Post`, `PerformanceRecord`, `TrustReview` | Free/Demo sehen `NetworkLocked` (Sprint 12; vorher `LockedArea`); nur echte, sichtbare Teilnehmer (sonst 404 – außer Kontakten/offenen Anfragen); Privatsphäre serverseitig (`06-permissions.md` §3d); Blockierung wirkt |
| `/app/people/demo/[key]` | **Vollständige Demo-Profilansicht (Sprint 7, Sprint 11 erweitert)** | Demo (`demoAccess`) oder member+ | WORKING | **keine DB** – reines `DEMO_PROFILES`-Datum aus `src/lib/demo` (+ Ziel-Taxonomie für Labels) | eindeutig als „DEMO · Beispielprofil" gekennzeichnet; „Kontakt anfragen" = simulierter Flow mit Pflichtnachricht und Demo-Abschluss (`DemoConnectDialog`, keine Action, keine DB-Zeile); nach Ablauf der Demo `LockedArea`; bei unbekanntem Key/`DEMO_CONTENT_ENABLED=false` → 404 |
...
| `network.ts` | `followAction`, `sendConnectionRequestAction` (**Pflichtnachricht ≥ 10 Zeichen**; Sprint 12: gegenseitige Anfrage ⇒ automatische Verbindung, 14 Tage Wartezeit nach Ablehnung, Empfänger muss echter Teilnehmer sein), `respondConnectionRequestAction` (Annehmen öffnet den Chat; Ablehnen ohne Mitteilung an den Absender), `withdrawConnectionRequestAction`, `disconnectAction`, `blockMemberAction` | Netzwerk-Flows | WORKING |
| `messages.ts` | `sendMessageAction`, `startConversationAction`, `markConversationReadAction`, `deleteMessageAction` (Sprint 12: `listMessageableContacts` entfernt) | Nachrichten (nur verbundene Konten; Schreiben mit `messaging`, sonst `networkAccessRequired`/`betaExpired`) | WORKING |
| `notifications.ts` | `markNotificationReadAction`, `markAllNotificationsReadAction`, `deleteNotificationAction`, `markRequestNotificationsSeenAction` (Sprint 12) | Mitteilungen | WORKING |
| `beta.ts` *(Sprint 12)* | `redeemBetaKeyAction`, `createBetaInviteAction`, `extendBetaAccessAction`, `revokeBetaAccessAction`, `disableBetaInviteAction` | Private Beta: Einlösen (Tester) und Verwaltung (Admin) | WORKING |
...
| `beta-redeem:user:<userId>` | 8 / Stunde | `actions/beta.ts` (Sprint 12) |
| `beta-redeem:ip:<gehashte IP>` | 30 / Stunde | `actions/beta.ts` (Sprint 12) |
...
| `/app/network` | Auth; echt für member/admin/**aktiven Beta-Zugang** | Verzeichnis, Suche/Rolle/Standort/Interessen, Follow/Connect | Sprint 12: **nur echte, geeignete Mitglieder** (keine Demo-Ergänzung mehr), Privatsphäre §3d; Trial: nur Demo-Profile; Beta beendet: Sperrseite bzw. Demo + Ende-Hinweis | „Kontakt anfragen“, „Zurückziehen“/„Annehmen“/„Ablehnen“ je Anfragezustand | WORKING | `networkDirectory` (member/admin/Beta) · Demo-Zweig `demoAccess` | Keine (K-06 für das Networking behoben) |
| `/app/discover` | Auth; echt für member/admin/**aktiven Beta-Zugang** | Business-Karten, regelbasiertes Ranking, Filter Rolle/Standort/Umkreis/Branche/Interesse/**Businessziel (Sprint 12)**/Ich suche/Ich biete/Investment/Typ | echt ohne Match-%/Kennzahlen/Trust; Trial: nur Demo; nie Demo als Auffüllung, ehrlicher Leerzustand | „Kontakt anfragen“ mit Pflichtnachricht | WORKING | `networkDiscover` · Demo-Zweig `demoAccess` | Keine |
| `/app/inbox` | Auth free (Schreiben: member/admin/aktiver Beta-Zugang) | Nachrichten, Anfragen, Benachrichtigungen, **Kontaktliste** (`?tab=requests&sub=connections`) | echt; Chat pollt alle 10 s; ein Chat pro Paar; nach Beta-Ende Verlauf nur lesbar | „Senden“, „Annehmen/Ablehnen“ | WORKING | Lesen: free · Schreiben/Annehmen: `messaging`/`connect` | Keine Fake-Unread; Badge = ungelesene Nachrichten + ungelesene Mitteilungen |
...
| `/app/beta` | Auth verifiziert | Beta-Schlüssel einlösen, Status (aktiv/abgelaufen/beendet), Hinweis für Mitglieder | echt (`BetaInvite`/`BetaAccess`) | „Beta-Zugang aktivieren“ | WORKING (Sprint 12) | verifiziert; Mitglieder/Admins: kein Formular | Keine |
| `/admin/beta` | Auth **admin** | Schlüssel erstellen (Klartext einmal), Einlösestatus, verlängern, vorzeitig beenden, deaktivieren | echt | „Schlüssel erstellen“ | WORKING (Sprint 12) | `User.role = admin` (Layout + jede Action) | Keine |
