...
| `/app/discover` | **Discover** – Business-Karten, Relevanz-Ranking, Filter `?role=&location=&industry=&interest=&kind=` | trial+ | WORKING | `listDiscoverCandidates()` + `src/lib/discover/matching.ts` (Interessen, Ziele, Branche, Standort, Suche/Biete) | ohne `entitlements.networkDiscover` ⇒ Redirect `/app/billing?paywall=trial`; Connect nur mit Pflichtnachricht |
...
| `/app/profile` | **Profil-Hauptbereich** – Header + `?tab=overview\|activity\|performance\|offers` | free | WORKING | `Profile`, `User`, `Post`, `Follow`, `Connection`, `TrustScoreSummary`, `PerformanceRecord`, `BusinessOpportunity`, `MarketplaceListing`, `InvestmentOpportunity`, `PrivacySettings` | Trust & Performance, Konto-Links und Profilfortschritt leben hier |
