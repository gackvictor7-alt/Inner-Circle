...
| --- | ----- | ------- | ------ | ----- | ------------------------- |
| `/` | Startseite (Hero, Säulen, Stats, Events, Membership, FAQ, CTA) | public | WORKING | liest `PlatformMetric` aus D1 (Fehler ⇒ Statistik-Sektion entfällt) | `force-dynamic`; Layout liest Access-Level für Header-CTA |
...
| `/membership` | Preismodell 24,99 €/Monat, Leistungen, Trust-Rechtliches | public | WORKING | keine DB | Jahrespreis im Marketingtext noch „folgt" (K-03) |
...
| `/app` | Dashboard (Begrüßung, Profilstand, Kennzahlen, Activity, Termine) | free | WORKING | `Connection`, `Follow`, `Post`, `BusinessOpportunity`, `TrustScoreSummary`, `PerformanceRecord`, `Conversation`, `Message`, `Notification`, `Event` | Trial-Banner + Countdown; Datenschutz-/Verifizierungshinweise |
| `/app/network` | Mitgliederverzeichnis, Suche/Filter, Follow/Connect | trial+ | WORKING (free sieht Locked-State) | liest `User`/`Profile`/`Follow`/`ConnectionRequest`, `Interest` | `listDirectoryMembers()` respektiert `discoverable`; Sichtbarkeit je Privacy teilweise (K-06) |
| `/app/discover` | Swipe-Discovery | trial+ | WORKING | wie Verzeichnis | ohne `entitlements.networkDiscover` ⇒ Redirect `/app/billing?paywall=trial` |
...
| `/app/notifications` | In-App-Mitteilungen | free | WORKING | `Notification` | eigene Datensätze; Aktionen prüfen `userId` |
| `/app/profile` | eigenes Profil, Statistiken, Trust-Auszug | free | WORKING | `Profile`, `User`, `Post`, `Follow`, `Connection`, `TrustScoreSummary` | – |
| `/app/profile/edit` | Profil bearbeiten | free | WORKING | `Profile`, `User` | Rate-Limit 40/h; Trial darf Basis-Felder bearbeiten |
| `/app/settings` | Datenschutz, Benachrichtigungen, Blockierte, Löschantrag, Dev-Link | free | WORKING | `PrivacySettings`, `NotificationPreference`, `Block`, `AccountDeletionRequest` | Löschung ist ein Antrag (manuelle Bearbeitung), keine Sofortlöschung |
...
| `/app/trust` | Trust & Performance (eigene Sicht) | trial+ | PARTIAL | `TrustScoreSummary`, `TrustReview`, `PerformanceRecord` | ohne Bewertungen bleibt der Score leer; Abgabe bewusst nicht aktiv (K-08) |
...
| `/app/opportunities/new` | Chance anlegen | member | WORKING | schreibt `BusinessOpportunity` | Ratelimit 10/h; `entitlements.opportunitiesManage` |
...
| `/app/marketplace/new` | Listing anlegen | member | WORKING | schreibt `MarketplaceListing` (+ `Course` bei `kind=course`) | Ratelimit 10/h; **kein** Verkäuferfreigabe-Gate (K-07b) |
...
| `/app/events` | Events im Mitgliederbereich | free | WORKING | `Event`, `EventApplication` | Liste zeigt nur veröffentlichte Zustände; ohne Daten ehrlicher Leerzustand |
...
| `network.ts` | `followAction`, `sendConnectionRequestAction`, `respondConnectionRequestAction`, `withdrawConnectionRequestAction`, `disconnectAction`, `blockMemberAction` | Netzwerk-Flows | WORKING |
...
| `profile.ts` | `updateProfileAction`, `updatePrivacyAction`, `updateNotificationPreferencesAction`, `skipInterestsAction`, `markOnboardingComplete`, `requestAccountDeletionAction` | Profil/Einstellungen | WORKING |
